 ...
 * @Date 2020/8/5
 */
public class BasicFileUDF extends UDF {
    ...
    public String evaluate(String line, String jsonkeysString) {
        ...
        //2.校验合法性
        if (logContents.length != 2 || StringUtils.isBlank(logContents[1]) || StringUtils.isBlank(logContents[0])) {
            return "";
        }
        ...
        try {
            ...
            //获取cm里面的对象
            JSONObject basic = jsonObject.getJSONObject("cm");

            //循环遍历取值
            for (int i = 0; i < jsonkeys.length; i++) {
                String filedName = jsonkeys[i].trim();
                if (basic.has(filedName)) {
                    sb.append(basic.getString(filedName)).append("\t");
                } else {
                    sb.append("\t");
                }
            }
    ...
    public static void main(String[] args) {
        ...
        String x = new BasicFileUDF().evaluate(line, "mid,uid,vc,vn,l,sr,os,ar,md,ba,sv,g,hw,nw,ln,la,t");
