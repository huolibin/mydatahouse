package com.haoxin.appclient;

import com.alibaba.fastjson.JSON;
import com.alibaba.fastjson.JSONArray;
import com.alibaba.fastjson.JSONObject;
import com.haoxin.bean.*;
...
public class AppMain {
    ...
    public static void main(String[] args) {
        ...
        //参数一：控制发送每条的延迟时间，默认是0
        Long delay = args.length > 0 ? Long.parseLong(args[0]) : 1000L;

        //参数二：循环遍历的次数
        int loop_len = args.length > 1 ? Integer.parseInt(args[1]) : 1000;

        //生产数据
        generteLog(delay,loop_len);
    }


    private static void generteLog(Long delay,int loop_len) {
